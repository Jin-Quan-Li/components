...
export default {
    ...
    lunarByBetween : function(year, between) {
        ...
        lunarArray.push(year, `${t > 9 ? t : `0${t}`}`, `${e > 9 ? e : `0${e}`}`); //年 月 日
    ...
    toLunar : function(year, month, day) {
        var yearData = this.lunarInfo[year - this.MIN_YEAR];
        if (year == this.MIN_YEAR && parseInt(month) <= 2 && parseInt(day) <= 9) {
            return [1891, 1, 1, '辛卯', '兔', '正月', '初一'];
        }
        return this.lunarByBetween(year, this.betweenSolarDays(year, parseInt(month), parseInt(day), yearData[1], yearData[2]));
    },
    //转换公历
    //@param year  阴历-年
    ...
    toSolar : function(year, month, day) {
        var yearData = this.lunarInfo[year - this.MIN_YEAR];
        var between  = this.betweenLunarDays(year, parseInt(month), parseInt(day));
